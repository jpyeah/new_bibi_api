...
//        /$ar->appRegister();
    ...
    public function updateuserpassAction(){
        exit;
        $sql="SELECT user_id,mobile FROM bibi_user WHERE mobile LIKE '%1888888%'  ";
    ...
    public function userAction(){
          ...
          $arr=array();
          for ($x=10; $x<=40; $x++) {
               array_push($arr,'bibi'.$x);
               
          } 
          for($y=100;$y<=130;$y++){
               array_push($arr,'bibi'.$y);
          }
          $db=new PdoDb;
          $num=md5('bibicar');
          foreach($arr as $k =>$val){
            //$sql="select username from `bibi_user` where `mobile` = "."'".$val."'";
            $sql = '
                    UPDATE
                    `bibi_user`
                    SET
