...
//        }
    ...
    public function sysMessageAction($type=1){
        ...
        elseif($type == 2){
            ...
            $carInfo = $carModel->GetCarInfoById('5762e10a94a11');
    ...
    public function meituanAction(){
         /* 
         $pdo = new PdoDb;
         $sql="select parent,count(*) AS count from meituancat group by parent order by count DESC ";
         $list=$pdo->query($sql);
             foreach($list as $k =>$val){
                     print_r($val['parent']);
                     $data['name']=$val['parent'];
                     $data['created_at']=time();
                     $data['updated_at']=time();
                     $data['parent_id']=1;
                     $data['parent_ids']=1;
                     $res=$pdo->insert('shop_category',$data);
                     //$sql='insert shop_category (name,created_at,updated_at,parent_id,parent_ids) values($name,$time,$time,)';
             }
          */
         $pdo = new Pdodb();
         $sql="select name,id from shop_category where type =1 ";
         $list=$pdo->query($sql);
         foreach($list as $k =>$val){
                 //print_r($val['name']);
                 $sqlt='select * from meituancat where parent ='."'".$val['name']."'";
                 $arr=$pdo->query($sqlt);
                 foreach($arr as $j =>$vale){
                      
                      $ttr['name']=$vale['catgory'];
                      $ttr['created_at']=time();
                      $ttr['updated_at']=time();
                      $ttr['parent_id'] =$val['id'];
                      $ttr['parent_ids'] ='1,'.$val['id'];
                      $ttr['type']=2;

                      $sl=$pdo->insert('shop_category',$ttr);
    ...
    public function categoryAction(){
        ...
        $sql="select * from product_category where type = 3 ";

        $list=$pdo->query($sql);

        foreach($list as $k){
                $where['id']=$k['id'];
                $update['root']=$k['parent_id'];
                $update['lft'] =1;
                $update['rgt']=2;
                $update['lvl']= 2;
                $pdo->updateByPrimaryKey('product_category',$where,$update);
        }
    ...
    public function updatecategoryAction(){
        ...
        $sql="select * from product_category where type = 1 ";

        $list = $pdo->query($sql);

        foreach($list as $k ){

            $num=$this->categorycheck($k['id'],2);
    ...
    function categorycheck($id,$num){
        ...
        if($list){
            ...
            foreach($list as $k ){
                ...
                $num = $this->categorycheck($k['id'],$num);
