...
class PdoDb
{
    ...
    static $w_dns = null;
    //mysql 用户名
    static $w_username = null;
    //mysql 密码
    static $w_password = null;
...
//        exit;
        ...
        self::$w_dns      = Yaf_Registry::get('config')->db->database->w_dns;
        self::$w_username = Yaf_Registry::get('config')->db->database->w_username;
        self::$w_password = Yaf_Registry::get('config')->db->database->w_password;
    }
    /**
     * PDO对象实例化
     * @return null|PDO
     */
    static function instance($sql="")
    {
        if(is_null(self::$pdo))
        {
            ...
            if(strstr($sql,"SELECT") || strstr($sql,"select"))
            {
                ...
                try
                {
                    self::$pdo = new PDO(self::$w_dns, self::$w_username, self::$w_password);
                    self::$pdo->setAttribute(PDO::ATTR_EMULATE_PREPARES, false);
                    self::$pdo->setAttribute(PDO::ATTR_STRINGIFY_FETCHES, false);
                    self::$pdo->query('set names utf8mb4');
                }
