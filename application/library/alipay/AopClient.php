...
class AopClient {
	...
	*/
	...
	public function toexecute($request){
        ...
        $strParams=array_merge($apiParams, $sysParams);
        ksort($strParams);
        
		//签名
	
        $strParams['sign']= $this->generateSign($strParams, $this->signType);
        
		//系统参数放入GET请求串
		$Orderstr='';
		foreach ($strParams as $strParamKey => $strParamValue) {
			$Orderstr .="$strParamKey=".urlencode($this->characet($strParamValue, $this->postCharset)) . "&";
		}
		$Orderstr = substr($Orderstr, 0, -1);
		return $Orderstr;
	}

	public function refundexecute($request){
