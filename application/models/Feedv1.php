...
class Feedv1Model extends PdoDb
{
       ...
       */
        ...
        if($feed['post_files']){
            ...
            if($feed['feed_type'] == 6){
                ...
                $items['content_info']=$feed['post_files'];
    ...
    public function getFeeds($type=1,$page = 1,$userId = 0)
    {
        ...
        $sql = '
            SELECT
            t1.feed_id,t1.feed_type,t1.user_id,t1.grade_id,t1.post_content,t1.image_url,t1.visit_num,t1.comment_num,t1.like_num,t1.collect_num,t1.feed_from,t1.created,t1.post_files,
            t2.avatar ,t2.nickname 
...
//                  if($this->keyword){
//                      $where .= ' AND t1.post_content LIKE "%'.$this->keyword.'%" ';
//                  }
                  $total = $this->query($sqlHotCnt)[0]['total'];
