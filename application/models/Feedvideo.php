 ...
 * note: 文章表
 */
class FeedvideoModel extends PdoDb
{
    ...
    public function handlerFeed($feed,$userId){
        ...
        $items['image_url']     = $feed['image_url'];
    ...
    public function getFeeds($type=1,$page = 1,$userId = 0)
    {
        ...
        $sql = '
            SELECT
            t1.feed_id,t1.feed_type,t1.user_id,t1.grade_id,t1.post_content,t1.image_url,t1.html_url,t1.visit_num,t1.comment_num,t1.like_num,t1.collect_num,t1.feed_from,t1.created,t1.post_files,
            t2.avatar ,t2.nickname 
    ...
    public function handleFeeds($feeds,$userId = 0){
        ...
        foreach ($feeds as $k => $feed) {
            ...
            $items[$k]['visit_num'] = $feed['visit_num'];
            $items[$k]['like_num'] = $feed['like_num'];
            $items[$k]['created'] = $feed['created'];
            $items[$k]['html_url'] = $feed['html_url'];
            $items[$k]['image_url'] = $feed['image_url'];
